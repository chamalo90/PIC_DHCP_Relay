...
__CONFIG(__CONFIG1L, 0x9e);  /* !!! DISABLE XINST (bit6) or else unpredictable results... - disable WDT (bit0) (but may be enabled by software) !!! */
__CONFIG(__CONFIG1H, 0xf7);
__CONFIG(__CONFIG2L, 0xfd); /* HS oscillator, PLL enabled and under software control */
__CONFIG(__CONFIG2H, 0xf9);  /* low bits determine WDT period, here 2,5 sec - useless here cause WDT disabled */
...
//MLstatic void ProcessIO(void); // play with potentiometer (end of this file)
...
// The interruptlow keyword is used to work around the bug when using C18
	...
	#else
//ML	#pragma interruptlow LowISR
//ML	void LowISR(void)
     void LowISR(void) interrupt 2 //ML for sdcc
	#endif
	{
	    TickUpdate();
	}
	...
	#else
//ML	#pragma interruptlow HighISR
//ML	void HighISR(void)
     void HighISR(void) interrupt 1 //ML for sdcc
	#endif
	{
...
ML*/
	...
	// required by the UART configuration routines
        TickInit();
//ML	#if defined(STACK_USE_MPFS) || defined(STACK_USE_MPFS2)
//ML	MPFSInit();
//ML	#endif
...
/*ML    // Initiates board setup process if button is depressed 
	// on startup
    if(BUTTON0_IO == 0u)
    {
...
/*ML	// Initialize any application-specific modules or functions/
...
/*ML
		...
		ProcessIO(); //play with potentiometer (end of this file)
ML*/
...
//ML			DisplayIPValue(AppConfig.MyIPAddr);
				DisplayIPValue(&(AppConfig.MyIPAddr)); //ML
...
//MLvoid DisplayIPValue(IP_ADDR IPVal)
...
//	printf("%u.%u.%u.%u", IPVal.v[0], IPVal.v[1], IPVal.v[2], IPVal.v[3]);
	...
	{
//ML	    uitoa((WORD)IPVal.v[i], IPDigit);
	    IP_field =(unsigned int)(*IPVal).v[i]; //ML
...
//	}
ML*/
//ML using sdcc and MAC Address programmed by manufacturer
	AppConfig.MyMACAddr.v[0] = 0;
	AppConfig.MyMACAddr.v[1] = 0;
	AppConfig.MyMACAddr.v[2] = 0;
